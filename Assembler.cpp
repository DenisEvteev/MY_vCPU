...
int My_Assembly::Tokenization()
{
    ...
    int length = 0;
    int real_len = 0;
    FILE* file = fopen("/home/denis/CLionProjects/Compilator/solv_electrical_circuit.txt", "r");
    if (file == nullptr)
    {
    ...
    /*fstream file_with_acm_;
    file_with_acm_.open("ishodnik.txt", fstream::in);
    if (!file_with_acm_.is_open())
    {
            cout << "ishodnik.txt Cannot be open" << endl;
            return ERROR_IN_FILE;
    }
    copy(istream_iterator<char>(file_with_acm_), istream_iterator<char>(), back_inserter(text));
    file_with_acm_.close();
