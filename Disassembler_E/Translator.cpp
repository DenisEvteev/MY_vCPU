...
void Translator::Generate_Code(){
    ...
    while(ip != size_code)
    {
        ...
        {
            case T_FUNC: {
                (this->*print_type[T_FUNC - T_NOL])(ip + 1, file_for_code);
                ip += 2;
                break;
            }

            case T_LABEL:{
                (this->*print_type[T_LABEL - T_NOL])(ip + 1, file_for_code);
                ip += 2;
                break;
            }

            case T_REG:{
                (this->*print_type[T_REG - T_NOL])(ip + 1, file_for_code);
                ip += 2;
                break;
            }

            case T_N:{
                (this->*print_type[T_N - T_NOL])(ip + 1, file_for_code);
                ip += 2;
                break;
            }
            case T_ADR:{
                (this->*print_type[T_ADR - T_NOL])(ip + 1, file_for_code);
                ip += 2;
                break;
            }
